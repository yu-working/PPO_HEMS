...
class HEMSEnvironment:
    """家庭能源管理系統環境類"""
    def __init__(self, pmv_up = 0.5, pmv_down = -0.5):
        # 舒適度約束條件
    ...
    def calculate_power(self, dehumidifier_humidity, 
        ...
        if ac_mode_weight[ac_mode] == 1:
            ac_consumption = ac_base_power * ac_mode_weight[ac_mode] * (1 + 0.1 * max(0, self.indoor_temp-float(ac_temperature))) * ac_fan_weight[ac_fan_speed] 
        else: 
...
class PPOAgent:
    ...
    def save_model(self):
        ...
        }, "./config/ppo_price_checkpoint.pt")
    
    def load_model(self, room_id='checkpoint'):
        """加載模型
        Args:
            path: 模型路徑
        """
...
if __name__ == "__main__":
    ...
    isa = pd.read_csv('./data/紅外線遙控器冷氣調控指令集.csv')
    ...
    Result.to_csv('./result/PPO用電分配參數測試結果.csv')
    end = time.time()
    print(end-start)
...
#%% 測試部分
    ...
    for i in range(14, 21):
        ...
        next_state, reward, done, env_state, device_state, pmv, cost = env.step(action, isa, step)
        pmv_test.append(pmv)
        Cost.append(cost)
        state = next_state
    ...
    # 保存測試結果
    pmv_test = pd.DataFrame(pmv_test)
    Result_test = pd.concat([env_state_test, device_state_test, pmv_test], axis = 1)
    Result_test.columns = ['env_temp', 'env_humd', 'dehumidifier', 'dehumidifier_hum',
                         'ac_temp', 'ac_fan', 'ac_mode', 'fan_on', 'pmv']
    Result_test['ac_temp'] = np.where(Result_test['ac_mode'] == 0, '-', Result_test['ac_temp'])
    ...
    Result_test.to_csv('./result/PPO用電分配參數應用結果.csv')
    end = time.time()
    print('time:',end-start)
#%% 計算節能效果

    # 讀取歷史數據
    data = pd.read_csv('./data/nilm_data_ritaluetb_hour.csv')
